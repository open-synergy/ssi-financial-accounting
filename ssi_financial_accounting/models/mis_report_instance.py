...
class MisReportInstance(models.Model):
    ...
    parent_menu_id = fields.Many2one(
        comodel_name="ir.ui.menu", domain=[("action", "=", False)], string="Parent Menu"
    )
    window_action_id = fields.Many2one(
        comodel_name="ir.actions.act_window", string="Action", copy=False, readonly=True
    )
    menu_id = fields.Many2one(
        comodel_name="ir.ui.menu", string="Menu", copy=False, readonly=True
    )
                ...
                .create(
                    {
                        ...
                        "view_id": self.env.ref(
                            "ssi_financial_accounting.mis_report_instance_launcher_form_view"
                        )
                        .sudo()
                        .id,
