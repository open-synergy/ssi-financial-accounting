...
class AgedPartnerBalanceReportWizard(models.TransientModel):
    ...
    use_auto_interval = fields.Boolean(string="Automatic Interval", default=True)
    due_interval = fields.Integer(string="Due Interval", required=False, default=30)
    interval_1 = fields.Integer(string="Interval I", required=False)
    interval_2 = fields.Integer(string="Interval II", required=False)
    interval_3 = fields.Integer(string="Interval III", required=False)
    interval_4 = fields.Integer(string="Interval IV", required=False)
    ...
    def get_interval_label(self):
        if self.use_auto_interval:
            ...
            for x in range(5):
                if x < 4:
                    start = x * self.due_interval + 1
                    end = (x + 1) * self.due_interval
                    label = f"{start} - {end} d."
                else:
    ...
    def _prepare_report_aged_partner_balance(self):
        res = super(
            AgedPartnerBalanceReportWizard, self
